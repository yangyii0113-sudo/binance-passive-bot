...
export function entryPlan(plan,{research=false}={}) {
  ...
  return `<div class="entry-plan" aria-label="進場、止盈與止損點位">
    ...
    <div class="entry-plan-exits">
      <div class="entry-plan-target"><span>第一止盈</span><strong>${quote(plan.tp1)} <small>USDT</small></strong><small>${research?`${plan.targetAnalysis?.tp1R?.toFixed(2)||'1'}R · 規劃平倉 50%`:'固定百分比參考'}</small></div>
      <div class="entry-plan-target"><span>第二止盈</span><strong>${quote(plan.tp2)} <small>USDT</small></strong><small>${research?`${plan.targetAnalysis?.tp2R?.toFixed(2)||'2'}R · 規劃平倉剩餘 50%`:'固定百分比參考'}</small></div>
    ...
    ${plan.targetAnalysis?`<div class="target-analysis"><strong>止盈依據</strong><p>${escape(plan.targetAnalysis.basis)}</p><p>前方結構：${quote(plan.targetAnalysis.structure)} USDT · 波動幅度 ATR：${quote(plan.targetAnalysis.atr)}</p><p>分批淨風報比：${plan.targetAnalysis.netRewardRisk.toFixed(2)}（兩段止盈均成交的情境，非預期收益）</p><p>假設單邊手續費 0.05%＋滑價 0.02%，未含資金費率。</p><p>第一止盈後，剩餘部位從下一根起移至成本保護停損；跳空仍可能虧損。</p></div>`:''}
  </div>`;
}
export function pullbackPanel(state,now=Date.now()) {
  const snapshot=state.pullback||{};
  return `<section class="pullback-panel" aria-label="TP01 趨勢回調研究"><div class="pullback-heading"><div><span class="research-kicker">BTC / ETH · 研究版 · 僅模擬</span><h3>TP01 趨勢回調 · 結構止盈</h3><p>4 小時辨識趨勢，1 小時等待回調收盤，再觀察突破進場。</p></div><button type="button" class="primary-btn" data-pullback-scan ${snapshot.loading?'disabled':''}>${snapshot.loading?'分析中…':'分析 BTC／ETH 進場點位'}</button></div>
...
function comparisonPanel(snapshot){
  ...
  return `<div class="exit-comparison"><h3>出場規則比較</h3><p>同一進場邏輯：原版 1R／2R 對照結構止盈＋成本篩選＋第一止盈後保護。前 70% 與後 30% 分開計算，固定參數、不自動挑選勝者。</p><div class="comparison-actions"><button type="button" class="primary-btn" data-pullback-compare="BTCUSDT" ${snapshot.comparing?'disabled':''}>${snapshot.comparing?'回測中…':'比較 BTC · 90 天'}</button><button type="button" class="primary-btn" data-pullback-compare="ETHUSDT" ${snapshot.comparing?'disabled':''}>比較 ETH · 90 天</button></div>
  ${snapshot.comparisonError?`<p role="alert">${escape(snapshot.comparisonError)}</p>`:''}
  ${r?`<h4>${escape(r.symbol)} · ${new Date(r.start).toISOString().slice(0,10)} 至 ${new Date(r.end).toISOString().slice(0,10)} UTC（結束不含）</h4>
  <p>各組起始 1,000 USDT，每筆風險預算 0.25%、名目本金上限 1 倍；單幣獨立研究。最長持有 48 根 1h，同根衝突先止損，區段末強制平倉。</p>
  ...
  <p>新版後段訊號 ${r.holdout.enhanced.signals} 次，空間／成本篩選略過 ${r.holdout.enhanced.skipped} 次。${Object.entries(r.holdout.enhanced.skipReasons||{}).map(([reason,count])=>`${escape(reason)}：${count} 次`).join('；')}</p>
