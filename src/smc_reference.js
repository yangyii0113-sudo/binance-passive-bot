...
export function smcReference(){
 return `<section class="smc-reference" aria-label="SMC 對照與翻車點">
 ...
 <div class="smc-check"><strong>進場前快速核對</strong><ul><li>同一幣種、同一週期？關鍵 K 棒是否已收盤？</li><li>區域與波段當時已確認？有沒有用後面的行情倒推？</li><li>寫清楚觸發價、失效點、有效期限；看到區域不等於已觸發。</li><li>前方支撐／壓力是否壓縮止盈？成本與部位風險是否已計入？</li></ul><p>三者重疊不等於三份獨立證據，也不代表已驗證的高勝率。維持僅模擬交易、真實下單鎖定。</p></div>
