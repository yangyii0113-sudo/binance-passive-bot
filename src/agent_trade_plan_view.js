...
export function agentTradePlanView(record, {symbol=record?.symbol || '', now=Date.now(), link=false}={}) {
  ...
  return `<section class="agent-trade-plan" aria-label="${name} 完整交易計畫" data-plan-symbol="${name}">
    <div class="agent-plan-heading"><div><span class="research-kicker">分析結果 → 交易計畫 · 僅模擬研究</span><h3>${name} · ${state.label}</h3></div><button type="button" class="secondary-btn" data-agent-plan-refresh="${name}" ${loading?'disabled':''}>${loading?'擬定中…':'重新產生計畫'}</button></div>
    ...
    ${state.plans.map(plan=>`<article class="agent-plan-scenario">
      ...
      <p>${esc(rules[plan.key])}</p>
      <p class="agent-plan-expiry">進場有效至 ${displayDate(plan.expiresAt)}，到時未觸發即取消。</p>
      ${entryPlan(plan,{research:true})}
      <details class="core-disclosure" data-search="${name} ${plan.key} 完整進退場規則"><summary>完整進退場規則與失效條件</summary><ol>
