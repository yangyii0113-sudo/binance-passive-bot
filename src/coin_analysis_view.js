...
export function coinAnalysisCards(snapshot,now,renderPlan){
 ...
 <div class="cards-grid coin-analysis-grid">${visible.map(row=>{
  ...
  if(!a){const stale=historical||row.expiresAt&&row.expiresAt<=now;return `<article class="detail-card"><strong>${esc(row.symbol)}</strong><p>${stale?'已過期，請重新分析':esc(row.reason||'請重新分析以取得三策略條件')}</p>${!stale&&row.status==='SETUP'?renderPlan(row,{research:true}):''}</article>`;}
  ...
  <button class="secondary-btn" type="button" data-pullback-compare="${esc(row.symbol)}" ${snapshot.loading||snapshot.comparing||historical?'disabled':''}>比較 ${esc(row.symbol)} 三策略 · 90 天</button>
  </article>`;
