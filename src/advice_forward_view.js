...
export function forwardTrackingBar(state,{full=false}={}){
  ...
  return `<section class="forward-control" aria-label="建議前向追蹤"><div><strong>${f.enabled?'本機前向追蹤中':'本機前向追蹤未啟動'}</strong><p>${esc(f.note||'開啟後再分析幣種，才會登錄新建議。')}</p></div>
    ...
    ${full&&f.feeds?.length?`<ul class="forward-feeds">${f.feeds.map(x=>`<li>${esc(x.symbol)} · ${x.status==='LIVE'?'即時行情已接通':x.status==='CONNECTING'?'正在連接行情':'行情中斷'}${x.lastAt?` · 最近狀態更新 ${displayDate(x.lastAt)}`:''}</li>`).join('')}</ul>`:''}
  </section>`;
}
...
export function adviceResultsPage(state){
  const f=state.forward||{},rows=f.book?.rows||[],s=forwardSummary(f.error?[]:rows),file=f.exportFile;
  ...
  return `<div class="page-stack forward-page">${section('建議成效',`
    ...
    <p class="guard-note">${f.error?'資料異常，停止計算成效。':'僅完整結案納入勝率；零損益也列入分母。無虧損樣本時不顯示獲利因子。少量樣本不能證明策略有效。'}</p>
    ...
    ${rows.length&&!f.error?`<details class="core-disclosure" data-search="forward-groups"><summary>依幣種／策略比較</summary><div class="forward-table-wrap"><table><thead><tr><th scope="col">組別</th><th scope="col">結案／登錄</th><th scope="col">勝率</th><th scope="col">平均淨損益</th><th scope="col">中斷／未追蹤</th></tr></thead><tbody>${grouped(rows,r=>r.symbol)}${grouped(rows,r=>FAMILY_NAMES[r.plan?.key]||'無可追蹤計畫')}</tbody></table></div><p>單位 USDT，未含資金費率；組別樣本少或缺漏多時不可排名。</p></details>`:''}
    <div class="history-actions"><button type="button" class="secondary-btn" data-forward-export ${!rows.length||f.error?'disabled':''}>匯出完整追蹤紀錄</button></div>
    ...
    <h3>逐筆追蹤 · 最新在前</h3>${f.error?'<p>紀錄異常，暫停展示逐筆成效；原始資料保留。</p>':rows.length?[...rows].reverse().map(r=>sampleCard(r,f.enabled)).join(''):'<div class="empty-state"><strong>尚無前向紀錄</strong><span>先開始本機前向追蹤，再回到交易建議重新分析幣種。舊研究紀錄不會變成成交。</span></div>'}
  `)}</div>`;
}
