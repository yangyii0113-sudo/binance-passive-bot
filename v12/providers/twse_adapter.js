...
const QUOTE_SOURCE='TWSE:STOCK_DAY_ALL',FLOW_SOURCE='TWSE:T86',REVENUE_SOURCE='TWSE:t187ap05_L';
...
function normalize(dataset,payload,context={}){if(dataset==='QUOTE')return normalizeDailyQuote(payload,context);if(dataset==='FLOW')return normalizeInstitutional(payload,context);if(dataset==='MONTHLY_REVENUE')return normalizeMonthlyRevenue(payload,context);throw Error('DATASET_UNSUPPORTED')}
module.exports=Object.freeze({descriptor,QUOTE_SOURCE,FLOW_SOURCE,REVENUE_SOURCE,normalizeDailyQuote,normalizeInstitutional,normalizeMonthlyRevenue,normalize});
