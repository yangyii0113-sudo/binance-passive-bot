...
const descriptor=Object.freeze({id:'cftc-cot-official',sourceLabel:'CFTC Commitments of Traders TFF Futures Only',markets:['GLOBAL','US'],capabilities:['FLOW','MACRO'],transport:'PUBLIC_READ_ONLY',executionWrite:false,priority:60,serverOnly:true});
...
function normalizeTffRow(row,{receivedAt}){
 ...
 return Object.freeze({entityId,contractCode:code,marketName:String(row.market_and_exchange_names??''),contractMarket:String(row.contract_market_name??''),commodityName:String(row.commodity_name??''),reportDate,knowledgeTime:'RECEIVED_AT',pointInTimeSafe:false,latencyClass:LATENCY_CLASS,evidenceRole:EVIDENCE_ROLE,source:SOURCE,observations:Object.freeze(defs.map(([f,v])=>obs(entityId,f,v,receivedAt)))});
}
function normalize(dataset,payload,context={}){if(dataset==='TFF_ROW')return normalizeTffRow(payload,context);if(dataset==='TFF_ROWS'){if(!Array.isArray(payload))throw Error('ROWS_REQUIRED');return Object.freeze(payload.map(x=>normalizeTffRow(x,context)));}throw Error('DATASET_UNSUPPORTED');}
module.exports=Object.freeze({descriptor,SOURCE,EVIDENCE_ROLE,LATENCY_CLASS,numeric,dateOnly,normalizeTffRow,normalize});
