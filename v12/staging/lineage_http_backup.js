...
function target(baseUrl,key){
  let base;
  try{base=new URL(baseUrl)}catch(_error){throw Error('LINEAGE_BACKUP_CONFIG_INVALID')}
...
async function backupLineageFileHttp({filePath,key,baseUrl,token}={}){
  ...
  let stat;
  try{stat=fs.statSync(filePath)}catch(_error){throw Error('LINEAGE_BACKUP_CONFIG_INVALID')}
  ...
  const put=await request({
    method:'PUT',url,filePath,
    headers:{authorization:auth,'content-length':String(stat.size),'x-foxyya-raw-size':String(stat.size),'x-foxyya-raw-sha256':sha,'content-type':'application/x-ndjson'}
  });
  ...
  if(remoteStatus!=='VERIFIED'||remoteSize!==stat.size||remoteSha!==sha||!Number.isInteger(compressedSize)||compressedSize<=0)throw Error('LINEAGE_BACKUP_VERIFY_FAILED');
  return Object.freeze({status:'VERIFIED',key,size:stat.size,sha256:sha,compressedSize,etag:String(head.headers.etag||put.headers.etag||'')});
}
