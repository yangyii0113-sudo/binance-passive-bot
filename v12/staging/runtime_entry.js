...
function runtimeConfig(env=process.env){
  ...
  const lineageFilePath=explicitLineagePath?env.FOXYYA_V12_LINEAGE_PATH.trim():'/data/foxyya-v12.lineage.jsonl';
  ...
  const forwardResearchFilePath=explicitForwardPath
    ...
    :(explicitLineagePath?path.join(path.dirname(lineageFilePath),'foxyya-v12-forward-research.forward.jsonl'):'/data/foxyya-v12-forward-research.forward.jsonl');
  if(!forwardResearchFilePath.endsWith('.forward.jsonl'))throw Error('FORWARD_LEDGER_PATH_INVALID');

  const rawRefresh=env.FOXYYA_V12_REFRESH_SECONDS===undefined||env.FOXYYA_V12_REFRESH_SECONDS===null||String(env.FOXYYA_V12_REFRESH_SECONDS).trim()===''?'1800':String(env.FOXYYA_V12_REFRESH_SECONDS).trim();
  ...
  return Object.freeze({
    host,port,lineageFilePath,
    lineageCompactThresholdBytes:lineagePolicy.compactionTriggerBytes,
    lineagePolicy,
    forwardResearchFilePath,refreshSeconds,
...
async function startFromEnvironment(env=process.env,dependencies={}){
  ...
  const onLineageMaintenance=dependencies.onLineageMaintenance||((result)=>{
    ...
    console.log('FOXYYA v12 lineage maintenance',JSON.stringify({
      status:result.status,trigger:result.trigger||null,retirementPending:result.retirementPending===true,
      activeBytes:after?.activeBytes??null,checkpointBytes:after?.checkpointBytes??null,
      totalBytes:after?.totalBytes??null,physicalBytes:after?.physicalBytes??null,
      usedRatio:after?.filesystem?.usedRatio??null,availableBytes:after?.filesystem?.availableBytes??null
    }));
  });
  ...
  const lineageStore=createDurableSourceLineageStore({filePath:config.lineageFilePath,now:clock,policy:config.lineagePolicy});
  ...
  const executionBridge=hasInjectedBridge?dependencies.executionBridge:(dependencies.fetchImpl===undefined?createExecutionReadBridge({fetchImpl}):null);
  ...
  async function close(){
    if(closed)return;
    closed=true;clearIntervalImpl(refreshTimer);await serverRuntime.close();
  }
  return Object.freeze({app:serverRuntime.app,server:serverRuntime.server,address:serverRuntime.address,lineageStore,forwardResearchStore,forwardResearchTracker,researchReady,close});
}
...
if(require.main===module){
  startFromEnvironment().then(runtime=>{
    ...
    runtime.researchReady.then(result=>{
      ...
      const published=result.orchestration?.published,home=published?.home;
    ...
    const shutdown=()=>{
      ...
      runtime.close().then(()=>process.exit(0)).catch(error=>{console.error('FOXYYA v12 staging shutdown failed:',error?.message||error);process.exit(1)});
    };
