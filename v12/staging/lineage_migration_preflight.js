...
function backupConfigured(config){
...
function backupKey(filePath,stat){
...
async function prepareDurableLineageStore({
  ...
  if(!nonEmpty(filePath)||typeof now!=='function'||!fsImpl||typeof fsImpl!=='object'||typeof backupLineageFileImpl!=='function'||typeof createStoreImpl!=='function')throw Error('LINEAGE_MIGRATION_CONFIG_INVALID');
  ...
  if(!needsMigration){
    ...
    return Object.freeze({store,migration:Object.freeze({status:'NOT_REQUIRED',backup:null,storageKind:kind})});
  }
  if(!backupConfigured(backupConfig))throw Error('LINEAGE_BACKUP_REQUIRED');
  const key=backupKey(filePath,before);
  const backup=await backupLineageFileImpl({
    filePath,
    key,
    endpoint:backupConfig.endpoint.trim(),
    region:backupConfig.region.trim(),
    bucket:backupConfig.bucket.trim(),
    accessKeyId:backupConfig.accessKeyId.trim(),
  ...
  const store=createStoreImpl({
    ...
    compactScratchDir:scratchDir,
    externalBackupVerified:true
  });
  return Object.freeze({
    ...
    migration:Object.freeze({
      ...
      compactedBytes:fsImpl.statSync(filePath).size,
