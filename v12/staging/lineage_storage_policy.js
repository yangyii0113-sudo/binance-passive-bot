...
function maintainLineageStorage({filePath,now=Date.now,fsImpl=fs,policy={}}={}){
  ...
  const filesystem=filesystemState(path.dirname(filePath),fsImpl);
  const softWater=filesystem!==null&&(filesystem.usedRatio>=normalized.softHighWaterRatio||filesystem.availableBytes<normalized.reserveBytes);
  ...
  if(!rotationDue&&!compactionDue&&!softWater){
    return Object.freeze({status:'NOOP',reason:'WITHIN_POLICY',activeBytes,checkpointBytes,filesystem,policy:normalized});
  }
...
module.exports=Object.freeze({DEFAULT_POLICY,normalizeLineagePolicy,capacityAssessment,assertLineageWriteCapacity,maintainLineageStorage});
