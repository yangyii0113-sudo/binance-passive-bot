(function(root,factory){
  ...
  function rankingHtml(row){
    if(!finite(row.researchScore))return '';
    const rank=Number.isInteger(row.rank)&&row.rank>0?`#${row.rank}`:'—';
    const priority=String(row.priority||'持續追蹤');
    const reasons=list(row.rankingReasons).filter(value=>typeof value==='string'&&value);
    return `<div class="research-ranking" data-ranking-purpose="${esc(row.rankingPurpose||'HUMAN_REVIEW')}"><div class="ranking-summary"><span>研究排名 <b>${esc(rank)}</b></span><span>研究分數 <b>${esc(Math.round(row.researchScore))}/100</b></span><strong>${esc(priority)}</strong></div><details class="ranking-details"><summary>排序依據</summary>${reasons.length?`<ul>${reasons.map(reason=>`<li>${esc(reason)}</li>`).join('')}</ul>`:'<p>尚無可解釋排序依據。</p>'}<small class="muted">此排名僅供人工研究覆核，不代表交易指令。</small></details></div>`;
  }
