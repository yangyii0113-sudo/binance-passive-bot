...
const markdown = [
  ...
  '',
  '| Symbol | TF | Range | Strategy | Bars | Trades | Win % | PF | Exp R | Net % | MDD % | Validation |',
  '|---|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---|',
  ...rows.map(row => `| ${row.symbol} | ${row.timeframe} | ${row.range} | ${row.strategyLabel} | ${row.samples ?? '—'} | ${row.trades ?? '—'} | ${safe(row.winRatePct)} | ${safe(row.profitFactor)} | ${safe(row.expectancyR,3)} | ${safe(row.netReturnPct)} | ${safe(row.maxDrawdownPct)} | ${row.error ? 'ERROR: '+row.error.replaceAll('|','/') : row.validation || '—'} |`),
  '',
