...
async function runWith(fetchImpl){
  ...
  try{
    ...
    const result=await pipeline.run(marketInput());
    return {result,store,cleanup:()=>fs.rmSync(dir,{recursive:true,force:true})};
  }catch(error){
...
test('source pipeline publishes Taiwan Market Core into TW pulse, TW region, lineage and diagnostics atomically',async()=>{
  ...
  const fixture=await runWith(async url=>table.get(url)||response(404,{}));
  try{
