...
test('home read model merges three active regional contexts, TW early trend, and read-only Crypto opportunities',()=>{
  ...
  assert.equal(out.home.regions.length,3);
  assert.equal(out.home.regions.find(x=>x.region==='TW').status,'AVAILABLE');
  assert.equal(out.home.regions.find(x=>x.region==='KR'),undefined);
