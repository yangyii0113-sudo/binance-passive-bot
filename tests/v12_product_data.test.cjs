...
test('completed source cycle publishes transport health and dataset diagnostics with Home',async t=>{
  ...
  const cftc=read.providerDiagnostics.providers.find(x=>x.providerId==='cftc-cot');
  assert.equal(cftc.health,'HEALTHY');
  const cftcDataset=read.providerDiagnostics.datasets.find(x=>x.datasetId==='CFTC:TFF:gpe5-46if:EQUITY_INDEX');
  assert.equal(cftcDataset.status,'AVAILABLE');
...
test('Global overview shows macro and positioning facts without inventing regional direction',async t=>{
  ...
  assert.equal(us.facts.find(x=>x.field==='positioning.cot.asset_manager.net').value,24000);
