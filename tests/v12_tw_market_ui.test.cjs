...
test('Taiwan market pulse renders broad advance in Traditional Chinese with index breadth and sector evidence',()=>{
  ...
  assert.match(html,/廣泛上漲/);
  assert.doesNotMatch(html,/BROAD_ADVANCE/);
  assert.match(html,/加權指數/);
  assert.match(html,/25,500/);
  assert.match(html,/0\.99%/);
  assert.match(html,/櫃買指數/);
  assert.match(html,/300/);
  assert.match(html,/1\.01%/);
  assert.match(html,/上漲家數/);
  assert.match(html,/1,300/);
  assert.match(html,/下跌家數/);
  assert.match(html,/400/);
  assert.match(html,/A\/D 比/);
  assert.match(html,/3\.25/);
  assert.match(html,/強勢類股/);
  assert.match(html,/半導體類/);
  assert.match(html,/弱勢類股/);
  assert.match(html,/鋼鐵類/);
  assert.match(html,/上櫃成交集中/);
  assert.match(html,/電子零組件業/);
  assert.match(html,/50\.11%/);
  assert.match(html,/成交比重，不代表類股漲跌幅/);
  assert.doesNotMatch(html,/市場指數資料仍待補齊/);
});
