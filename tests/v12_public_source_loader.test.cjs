...
test('decision-required and credential-required sources are blocked before network access',async()=>{
  ...
  const usSip=createPublicSourceLoader({sourceId:'us-equity-realtime',endpoint:'https://example.invalid/quote',fetchImpl});
  ...
  assert.equal((await usSip.load()).reason,'CREDENTIAL_REQUIRED');
