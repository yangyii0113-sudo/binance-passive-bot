...
function fixture(){
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'foxyya-lineage-p0-'));
  return {dir,filePath:path.join(dir,'foxyya-v12.lineage.jsonl')};
}
...
test('growth audit reports bounded storage metadata without canonical payloads',()=>{
  const {dir,filePath}=fixture();
  try{
    let now=2000;
    const store=createDurableSourceLineageStore({filePath,now:()=>now++});
    const quote=sourceRecord({receivedAt:1000,datasetId:'TPEX:quote'});
    ...
    assert.equal(audit.totalBytes,fs.statSync(filePath).size);
...
test('bounded compaction atomically keeps newest output dependency closure and restart trace',()=>{
  const {dir,filePath}=fixture();
  try{
    let now=5000;
    const store=createDurableSourceLineageStore({filePath,now:()=>now++});
    const oldSource=sourceRecord({receivedAt:1000,value:460});
    const oldOutput=outputRecord([oldSource],{asOf:1500});
    const newSource=sourceRecord({receivedAt:3000,value:468});
    ...
    const newestClosureBytes=lines.slice(-2).reduce((sum,line)=>sum+Buffer.byteLength(line)+1,0);
    const result=compactLineageStorage({filePath,now:()=>9000,policy:{retainedTargetBytes:newestClosureBytes+32,checkpointAuditRecords:8}});
...
test('compaction space failure leaves the current journal byte-identical',()=>{
  const {dir,filePath}=fixture();
  try{
    let now=7000;
    const store=createDurableSourceLineageStore({filePath,now:()=>now++});
    const source=sourceRecord({receivedAt:6000});
    store.recordSource(source);store.recordOutput(outputRecord([source],{asOf:6500}));
    const before=fs.readFileSync(filePath);
    const fsImpl=Object.create(fs);
    const realWrite=fs.writeSync.bind(fs);
    let writes=0;
    fsImpl.writeSync=(...args)=>{
      writes+=1;
      if(writes>=1){const error=Error('no space');error.code='ENOSPC';throw error;}
      return realWrite(...args);
    };
    const result=compactLineageStorage({filePath,fsImpl,now:()=>9000,policy:{retainedTargetBytes:1024*1024,checkpointAuditRecords:8}});
    assert.equal(result.status,'LINEAGE_COMPACTION_SPACE_REQUIRED');
    assert.deepEqual(fs.readFileSync(filePath),before);
  }finally{fs.rmSync(dir,{recursive:true,force:true})}
});

test('store maintenance is non-enumerable, rotates oversized journal, and rebuilds indexes',()=>{
  const {dir,filePath}=fixture();
  try{
    let now=10000;
    const store=createDurableSourceLineageStore({
      filePath,now:()=>now++,
      policy:{retainedTargetBytes:1024*1024,activeRotationBytes:1,compactionTriggerBytes:2*1024*1024,softHighWaterRatio:0.8,criticalHighWaterRatio:0.9,reserveBytes:0,checkpointAuditRecords:8}
    });
    const source=sourceRecord({receivedAt:9000,value:471});
    const output=outputRecord([source],{asOf:9500});
...
test('critical disk high-water rejects append before write and live indexes do not advance',()=>{
  const {dir,filePath}=fixture();
  try{
    const fsImpl=Object.create(fs);
    let writeCalls=0;
    ...
    const store=createDurableSourceLineageStore({
      filePath,now:()=>2000,fsImpl,
      policy:{retainedTargetBytes:1024,activeRotationBytes:2048,compactionTriggerBytes:4096,softHighWaterRatio:0.8,criticalHighWaterRatio:0.9,reserveBytes:0,checkpointAuditRecords:8}
    });
...
test('checkpoint audit retention preserves a verifiable removed-history chain anchor',()=>{
  const {dir,filePath}=fixture();
  try{
    let now=20000;
    const policy={retainedTargetBytes:1024*1024,activeRotationBytes:1,compactionTriggerBytes:2*1024*1024,softHighWaterRatio:0.8,criticalHighWaterRatio:0.9,reserveBytes:0,checkpointAuditRecords:2};
    ...
    assert.equal(typeof records[0].previousCheckpointDigest,'string');
