...
test('binding factory surface is fixed and contains no execution methods',()=>{
  const api=bindings();
  assert.deepEqual(Object.keys(api).sort(),['blsSeries','cftcTffEquityIndex','ecbSeries','secCompanyFact','tpexDailyQuote','tpexInstitutional','tpexMonthlyRevenue','twseDailyQuote','twseInstitutional','twseMonthlyRevenue']);
